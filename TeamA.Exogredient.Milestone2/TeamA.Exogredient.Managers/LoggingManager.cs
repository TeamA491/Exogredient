...
namespace TeamA.Exogredient.Managers
{
    public static class LoggingManager
    {
        ...
        {
            bool ffLoggingResult = await FlatFileLoggingService.LogToFlatFileAsync(timestamp, operation, identifier, ipAddress, errorType).ConfigureAwait(false);
            ...
            while (!(ffLoggingResult && dsLoggingResult) && count < Constants.LoggingRetriesAmount)
            {
                if (!ffLoggingResult)
                {
                    ffLoggingResult = await FlatFileLoggingService.LogToFlatFileAsync(timestamp, operation, identifier, ipAddress, errorType).ConfigureAwait(false);
                }
                if (!dsLoggingResult)
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (ffLoggingResult)
                    {
                        rollbackSuccess = await FlatFileLoggingService.DeleteFromFlatFileAsync(timestamp, operation, identifier, ipAddress, errorType).ConfigureAwait(false);
                    }
                    if (dsLoggingResult)
                    {
